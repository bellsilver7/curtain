...
- [x] 설계 문서 (`docs/design.md`) · 결정 기록 (`docs/adr/` 5건)
...
- [x] **결제 사가** (`app/service/order_service.py` + `app/infra/pg/`)
      — 무응답 복구, 확정 경로 하나, 멱등 3겹, outbox, 취소·환불 순서
- [ ] 대기열: ZSET 큐 · 입장 허용기 · locust

### 지금 통과하는 것

`make test` → 55건. Postgres 16 에 실제 동시 요청을 던져 확인했다.
...
| PG 무응답 + 실제 승인 | 사용자는 실패 응답, 리컨실러가 되물어 확정. 좌석 1건·결제 1건 |
| PG 무응답 + 미승인 | 주문 `CANCELED`, 좌석 복원, 승인된 결제 0건 |
| 되물어도 PG 가 모름 | 아무 결론도 내리지 않고 다음 tick 에 다시 묻는다 |
| 같은 멱등키 동시 5회 + 늦은 재시도 | 주문 1건·결제 1건, 응답 본문 전부 동일 |
| 같은 승인 콜백 3회 | `order_items` 1행, 3회 모두 성공, 알림 1건 |
| 승인 왕복 중 hold 만료 | 확정 안 함 + 자동 환불 |
| 환불 실패 시 취소 | 좌석 복원 안 함, 주문은 `PAID` 유지 |
| 남의 hold 로 주문 | 거절, PG 호출 0회 |

## 다음 한 걸음

정합성(선점·회수·한도), 그 앞의 부하 방벽(게이트), 읽기 경로의 캐시, 그리고 돈이
걸린 사가까지 증명됐다. 남은 것:

1. 대기열 — ZSET 큐와 입장 허용기. 유량 밸브 값(현재 200/s)을 부하 테스트로 확정한다
2. HTTP 계층 — 좌석맵의 etag 는 아직 서비스 반환값일 뿐이다. 304 응답으로
   쓰려면 라우터가 필요하다. 캐시 적중 한 건을 재보니 Redis GET 0.30ms 에
   `json.loads` 0.68ms + `SeatCell` 1,200개 조립 1.53ms 였다 — 적중 경로의
   88% 가 역직렬화다. 304 는 그 전부를 건너뛴다
3. 워커 루프 — 회수·리컨실·outbox 발행은 함수만 있고 tick 루프가 없다.
   `reconcile()` 은 테스트에서만 호출되고 있다
4. CI — `make drift` 와 `make test` 를 걸어둔다. `ruff` 는 지금 초록이다

### 테스트를 의심하는 방법

이 저장소의 테스트는 통과했다는 사실만으로는 못 믿는다. 구현 후 일부러
깨뜨려 빨강이 되는지 확인한다.

게이트에서 여섯 가지, 좌석맵에서 일곱 가지를 망가뜨려 전부 빨강을 확인했다.
그 과정에서 실제로 아무것도 검증하지 않던 테스트 하나를 찾아 고쳤고, 잘못
짚었던 원인 진단 두 건을 바로잡았다. 사보타주가 초록으로 남으면 두 가지 중
하나다 — 테스트가 약하거나, 사보타주가 도달 불가능한 코드를 건드렸거나.
후자가 더 흔했고, 그 자체로 배울 것이 있었다.
