...
ORM 세션은 쓰지 않는다. 실행은 전부 Core 이고(app/infra/db/queries.py), 이 파일은
세 가지 역할만 한다.

  1. Alembic autogenerate 가 diff를 뜨는 대상
  2. 컬럼·제약·인덱스가 한눈에 보이는 문서
  3. 쿼리가 참조하는 컬럼과 제약의 이름표

3번이 있어서 스키마를 바꾸면 어긋난 쿼리가 import 시점에 깨진다
(결정 기록: 쿼리 작성 방식). 여기서 이름을 바꿀 때는 queries.py 도 같이 본다.
