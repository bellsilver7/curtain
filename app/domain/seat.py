...
§4 의 전이 표가 그대로 코드가 되는 자리. 표준 라이브러리만 import 한다 (§9 계층 규칙).
DB 도 Redis 도 HTTP 도 모르므로, 규칙 자체는 인프라 없이 테스트할 수 있다.

이 모듈은 규칙을 *선언*하고, 실제 강제는 두 곳에서 이뤄진다.

  1. SQL 의 조건부 `WHERE status = 기대값` (원칙 03) — 동시성 하에서의 실제 방어
...
class SeatStatus(StrEnum):
    """DB 의 `seat_status` ENUM 과 1:1. 값을 바꾸면 마이그레이션이 필요하다 (ADR 0002)."""
...
#: 허용된 전이. 설계 문서 §4 의 표와 같은 내용이며, 테스트가 둘의 일치를 지킨다.
#:
#: 취소는 좌석 상태가 아니라 주문 상태(`orders.status = 'CANCELED'`)로 표현한다.
#: 좌석의 상태는 "지금 팔 수 있는가"만 답하면 되고, 환불 대기 중인 좌석은 팔 수
#: 없으므로 BOOKED 로 남는다 (§7.3: 좌석 복원은 환불 성공 확인 후). 그래서
#: seat_status ENUM 에 CANCELLED 가 없다 — 설계 문서 §4 다이어그램과 다른 지점이다.
...
def is_claimable(status: SeatStatus, hold_expired: bool) -> bool:
    """선점 대상인가. 만료된 hold 는 스윕 워커를 기다리지 않고 즉시 회수된다 (§5.2).
