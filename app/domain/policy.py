"""정책 상수와 순수 계산 — 설계 문서: 정책 상수, 취소와 환불
...
#: 예매 API 실측 처리량에서 역산한 입장 유량. 부하 테스트 결과에 따라 조인다 (대기열).
...
#: 폴링이 이 시간 이상 끊긴 대기자는 줄에서 뺀다 (대기열 이탈 감지).
QUEUE_HEARTBEAT_TIMEOUT = timedelta(seconds=15)

# ---------------------------------------------------------------- 결제

#: 주문이 PENDING으로 이 시간을 넘기면 리컨실러가 PG사에 되묻는다 (배치 워커).
PAYMENT_PENDING_LIMIT = timedelta(minutes=5)

#: hold 잔여가 이보다 적으면 결제 승인 요청 자체를 거절한다.
#: 승인 왕복 중에 hold가 만료되는 창을 좁히기 위한 장치 (좌석 상태 전이 참고).
...
class CancelClosed(Exception):
    """관람일 당일 이후로는 취소할 수 없다 (좌석 상태 전이 → 422 CANCEL_CLOSED)."""
...
def cancel_fee(ticket_amount: int, now: datetime, starts_at: datetime) -> int:
    """취소 수수료(원). 결과는 payments.fee_snapshot 에 그대로 남긴다 (취소와 환불).
